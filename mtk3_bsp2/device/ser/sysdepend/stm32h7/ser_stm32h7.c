...
#define CPU_STM32H7		1		/* Target CPU : STM32H7 series (bsp def) */
#include <sys/sysdepend/nucleo_h723/sysdef.h>
...
{
	ER	err	= E_OK;
	volatile unsigned long xxx = PCLK1;
...
//		ll_devcb[unit].speed = ((PCLK1*1000*1000) + parm/2)/parm;
		ll_devcb[unit].speed = (137500000 + parm/2)/parm;
		break;
